...
if true; then
    ...
    run_check "local_storage_upload" \
        sflow run "$EXAMPLES_DIR/local_storage_upload.yaml" --dry-run
    run_check "local_storage_upload_all" \
        sflow run "$EXAMPLES_DIR/local_storage_upload_all.yaml" --dry-run
    ...
    KUBERNETES_MULTINODE_DRYRUN_LOG="$BACKEND_AGNOSTIC_DIR/kubernetes_multinode_smoke.log"
    ...
    run_check "dry-run docker_hello_world uses docker_run default operator" \
        bash -c "sflow run \"$EXAMPLES_DIR/docker_hello_world.yaml\" --dry-run --verbose > \"$DOCKER_HELLO_DRYRUN_LOG\" 2>&1 && \
    ...
    run_check "dry-run docker_multi_node assigns remote Docker hosts and GPUs" \
        bash -c "sflow run \"$EXAMPLES_DIR/docker_multi_node.yaml\" --dry-run --verbose > \"$DOCKER_MULTI_DRYRUN_LOG\" 2>&1 && \
    ...
    run_check "dry-run kubernetes_hello_world uses k8s operator" \
        bash -c "sflow run \"$EXAMPLES_DIR/kubernetes_hello_world.yaml\" --dry-run --verbose > \"$KUBERNETES_HELLO_DRYRUN_LOG\" 2>&1 && \
    ...
    run_check "dry-run kubernetes_multinode_smoke uses k8s operator" \
        bash -c "sflow run \"$EXAMPLES_DIR/kubernetes_multinode_smoke.yaml\" --dry-run --verbose > \"$KUBERNETES_MULTINODE_DRYRUN_LOG\" 2>&1 && \
            grep -F -- 'id=kubernetes' \"$KUBERNETES_MULTINODE_DRYRUN_LOG\" && \
            grep -F -- 'operator: k8s' \"$KUBERNETES_MULTINODE_DRYRUN_LOG\" && \
            grep -F -- 'Dry-run complete: k8s_multinode_smoke' \"$KUBERNETES_MULTINODE_DRYRUN_LOG\""
    run_check "dry-run kubernetes_multinode uses k8s operator" \
        bash -c "sflow run \"$EXAMPLES_DIR/kubernetes_multinode.yaml\" --dry-run --verbose > \"$KUBERNETES_LWS_DRYRUN_LOG\" 2>&1 && \
    ...
    run_check "dry-run report uses standardized sections + compact tasks by default" \
        bash -c "sflow run \"$EXAMPLES_DIR/local_dag.yaml\" --dry-run > \"$DRYRUN_COMPACT_LOG\" 2>&1 && \
    ...
    run_check "dry-run --verbose expands full per-task detail" \
        bash -c "sflow run \"$EXAMPLES_DIR/local_dag.yaml\" --dry-run --verbose > \"$DRYRUN_VERBOSE_LOG\" 2>&1 && \
    ...
    run_check "dry-run renders storage targets and planned uploads" \
        bash -c "sflow run \"$EXAMPLES_DIR/local_storage_upload.yaml\" --dry-run > \"$DRYRUN_UPLOADS_LOG\" 2>&1 && \
    ...
    run_check "dry-run renders planned workflow upload_all archive" \
        bash -c "sflow run \"$EXAMPLES_DIR/local_storage_upload_all.yaml\" --dry-run > \"$DRYRUN_UPLOAD_ALL_LOG\" 2>&1 && \
...
EOF
    ...
    run_check "sflow batch dry-run Plan shows sbatch out/err paths" \
        bash -c "sflow batch \"$EXAMPLES_DIR/local_dag.yaml\" --partition batch --account acct --nodes 1 > \"$DRYRUN_BATCH_LOG\" 2>&1 && \
    ...
    run_check "run local_dag (live)" \
        sflow run "$EXAMPLES_DIR/local_dag.yaml" \
...
EOF
    ...
    RESULT_PARSING_SAMPLE="$EXAMPLES_DIR/local_result_parsing.yaml"
...
with mock_aws():
    ...
    per_task_out = SflowApp().run(
        file=repo / 'examples' / 'local_storage_upload.yaml',
        dry_run=False,
        workspace_dir=repo,
    ...
    upload_all_out = SflowApp().run(
        file=repo / 'examples' / 'local_storage_upload_all.yaml',
        dry_run=False,
        workspace_dir=repo,
...
EOF
    ...
    run_check "run fake slurm multi-backend binds each task to its own backend" \
        bash -c "set -euo pipefail
            ...
            unset SLURM_JOB_ID SLURM_JOBID SLURM_JOB_NODELIST SLURM_NODELIST SLURM_NNODES SLURM_HET_SIZE
            rm -f \"\$SFLOW_FAKE_SLURM_LOG_DIR\"/*.args
            sflow run '$EXAMPLES_DIR/slurm_multi_backend.yaml' \
...
EOF
    ...
    run_check "batch sbatch venv bootstrap script generation" \
        sflow batch -f "$EXAMPLES_DIR/local_dag.yaml" \
    ...
    run_check "run --enable-workflow-monitor injects a workflow monitor (dry-run)" \
        bash -c "sflow run \"$EXAMPLES_DIR/local_dag.yaml\" --enable-workflow-monitor --dry-run > \"$MONITOR_CLI_WF_LOG\" 2>&1 && \
    ...
    run_check "run --enable-task-monitor injects per-task monitors (comma/space/repeat list, dry-run)" \
        bash -c "sflow run \"$EXAMPLES_DIR/local_dag.yaml\" --enable-task-monitor 'prepare_data,preprocess' --enable-task-monitor 'train export_model' --dry-run > \"$MONITOR_CLI_TASK_LOG\" 2>&1 && \
    ...
    run_check "run --enable-task-monitor rejects an unknown task" \
        bash -c "! sflow run \"$EXAMPLES_DIR/local_dag.yaml\" --enable-task-monitor nope_task --dry-run > \"$MONITOR_CLI_BAD_LOG\" 2>&1 && \
    ...
    run_check "batch threads monitor flags into the sbatch script + composed snapshot" \
        bash -c "sflow batch -f \"$EXAMPLES_DIR/local_dag.yaml\" -p \"$PARTITION\" -A \"$ACCOUNT\" --nodes 1 --log-level warn --enable-workflow-monitor --enable-task-monitor train -o \"$MONITOR_CLI_BATCH_SCRIPT\" > \"$MONITOR_CLI_BATCH_LOG\" 2>&1 && \
...
if [ -n "$SUBMIT" ] && [ -z "$PREFLIGHT_ONLY" ]; then
    ...
    E2E_BATCH_EXTRA_ARGS=(
        ...
        "-e --exclude=gb-nvl-137-compute09,gb-nvl-137-compute16,gb-nvl-137-compute03"
    )
