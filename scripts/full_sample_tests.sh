...
fetch_infmax_recipes() {
    ...
    # A pinned COMMIT is the point of the freeze, but `git clone --branch` only
    # accepts a branch or tag -- given a SHA it fails with "Could not find remote
    # branch". So a SHA-shaped ref takes the blobless clone + checkout path
    # instead; branches and tags keep the cheap --depth 1 path.
    local clone_ok=0
    if printf '%s' "$INFMAX_RECIPE_REF" | grep -Eq '^[0-9a-f]{7,40}$'; then
        if GIT_TERMINAL_PROMPT=0 git clone --filter=blob:none --no-checkout \
                "$INFMAX_RECIPE_REPO" "$tmp_dir/repo" >/dev/null 2>&1 \
           && git -C "$tmp_dir/repo" checkout --detach "$INFMAX_RECIPE_REF" >/dev/null 2>&1; then
            clone_ok=1
        fi
    elif GIT_TERMINAL_PROMPT=0 git clone --depth 1 --branch "$INFMAX_RECIPE_REF" \
            "$INFMAX_RECIPE_REPO" "$tmp_dir/repo" >/dev/null 2>&1; then
        clone_ok=1
    fi
    ...
    echo "  recipes at $(git -C "$tmp_dir/repo" rev-parse HEAD)"
...
if true; then
    ...
    # Also asserts the dry-run names the report folders it will write and marks
    # which are marker-windowed -- the listing is what shows a task monitor's
    # marker window before the run, so a typo'd marker is visible here instead of
    # after the run as an empty report.
    LOCAL_MONITOR_DRYRUN_LOG="$PREFLIGHT_DIR/local_monitor_dryrun.log"
    run_check "local_monitor dry-run lists marker-windowed report folders" \
        bash -c "sflow run \"$EXAMPLES_DIR/self_contained/local/monitor.yaml\" --dry-run > \"$LOCAL_MONITOR_DRYRUN_LOG\" 2>&1 && \
            grep -F -- 'report folders:' \"$LOCAL_MONITOR_DRYRUN_LOG\" && \
            grep -F -- \"windowed/work/  (log-marker window: start='MEASURE_START' (first), end='MEASURE_END' (last))\" \"$LOCAL_MONITOR_DRYRUN_LOG\" && \
            grep -F -- 'lifecycle/warmup/  (task lifecycle window)' \"$LOCAL_MONITOR_DRYRUN_LOG\""
    # Docker GPU monitor example: dry-run only, so this validates the
    # monitor.window schema on a CPU-only runner (nbody never executes here).
    run_check "docker_gpu_monitor" \
        sflow run "$EXAMPLES_DIR/self_contained/docker/gpu_monitor.yaml" --dry-run
    ...
    DOCKER_GPU_RESERVATION_DRYRUN_LOG="$BACKEND_AGNOSTIC_DIR/gpu_reservation_demo.log"
    DOCKER_GPU_HOG_DRYRUN_LOG="$BACKEND_AGNOSTIC_DIR/gpu_reservation_hog.log"
    DOCKER_GPU_PIPELINE_DRYRUN_LOG="$BACKEND_AGNOSTIC_DIR/gpu_reservation_pipeline.log"
    DOCKER_GPU_SCHEDULING_DRYRUN_LOG="$BACKEND_AGNOSTIC_DIR/gpu_reservation_scheduling.log"
...
sglang_serve on docker|docker_run|-s GPUS_PER_NODE=1 -s SERVER_GPUS=1|backends/docker.yaml workloads/sglang_serve.yaml benchmark.yaml
vllm_serve on docker|docker_run|-s GPUS_PER_NODE=2 -s SERVER_GPUS=2|backends/docker.yaml workloads/vllm_serve.yaml benchmark.yaml
dynamo_vllm on docker|docker_run|-s GPUS_PER_NODE=2 -s SERVER_GPUS=2|backends/docker.yaml workloads/dynamo_common.yaml workloads/dynamo_vllm.yaml benchmark.yaml
MODULAR_CASES
