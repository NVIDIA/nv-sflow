...
> **Kubernetes log early-stop:** Kubernetes prioritizes pod execution over log delivery, so `kubectl logs -f` can lag minutes behind a pod that has already finished. sflow follows the logs live (into `<task>.log`) but watches the pod phase, and the moment a pod is terminal it stops draining that backlog so the task's status syncs with the pod immediately instead of waiting on the stream. To avoid losing the still-undelivered tail, it first dumps the **complete** container log to a temporary `<pod>.pod.log` (a direct `kubectl logs` that bypasses the per-line driver pipeline). Once the task is done, sflow swaps that complete copy in as the final `<task>.log` and deletes the temp — so `<task>.log` is the single, complete record (aligned with the other backends), with no duplicate file. The console/live view may show a `[sflow] <pod>: pod finished ... stopped live log stream early` line where the live stream was cut; the on-disk `<task>.log` still ends up complete.
