...
| `group` | no | `value` then `1` | Capture group name or index to extract. |
...
- A file already in `sflow.result.v1` shape is validated and rewritten as-is.
...
- **Type casting**: `type: auto` infers `int`, then `float`, then `bool`
  ...
  Explicit-type failures are recorded under `errors`.
...
See `examples/local_result_parsing.yaml`, which exercises both the regex-map and
