...
- **Passive.** Monitors overlap the workload (`srun --overlap`) and never reserve
  nodes or GPUs.
...
| `disk` | `/` usage (`statvfs`) |
| `network` | rx/tx bytes & packets and per-second rates (`/proc/net/dev`) |
...
| Path | Contents |
|------|----------|
| `<run>/sflow_monitor.log` | Terminal-friendly overview: per-metric min/avg/max tables, ASCII sparkline timelines, per-monitor windows, and the list of per-task reports. Sibling to `sflow_summary.log`. |
