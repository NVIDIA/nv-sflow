...
All `sflow.yaml` config fields at a glance. **Bold** names are required.
...
| **`workflow`** | object | — | Workflow definition containing name and tasks. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`value`** | any | — | Variable value (int, float, bool, string, or list). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`uri`** | string | — | Resource URI with scheme (`fs://`, `file://`, `http://`, `s3://`). |
| `description` | string | `null` | Human-readable description. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`account`** | string / expr | — | Slurm account. |
| **`partition`** | string / expr | — | Slurm partition. |
| **`time`** | string / expr | — | Time limit (e.g. `00:30:00`). |
| **`nodes`** | int / expr | — | Number of nodes. |
| **`gpus_per_node`** | int / expr | — | GPUs per node. |
| `extra_args` | list[string] | `[]` | Extra `salloc` arguments (e.g. `--exclusive`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`type`** | string | — | Operator type: `bash`, `srun`, `docker`, `ssh`, or `python`. |
...
| **`tasks`** | list | — | List of task definitions (must be non-empty). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`name`** | string | — | Task name (must be unique). |
| **`script`** | list[string] | — | Script lines to execute (non-empty). |
...
| **`gpus.count`** | int / expr | — | Number of GPUs (sets `CUDA_VISIBLE_DEVICES`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`count`** | int / expr | — | Number of retries. |
| **`interval`** | int / expr | — | Delay between retries (seconds). |
...
| Probe Type | Key Fields | Description |
|------------|------------|-------------|
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| **`pattern`** | string | — | Parse pattern (e.g. `"TTFT: {ttft:f} ms"`). |
