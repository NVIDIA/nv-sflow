...
class Backend(ABC):
    ...
    def resource_env(self, *, cuda_visible_devices: str | None = None) -> dict[str, str]:
        ...
        if cuda_visible_devices is None or not self.capabilities.supports_gpu_env:
            return {}
        # Mirror the GPU slice onto NVIDIA_VISIBLE_DEVICES as well: some platforms and
        # newer GPU/driver stacks (and the NVIDIA container runtime) honor only that
        # variable, so a task pinned via CUDA_VISIBLE_DEVICES alone could see the wrong
        # devices. Keeping the two identical is a no-op where CUDA_VISIBLE_DEVICES already
        # works and a fix where it doesn't.
        return {
