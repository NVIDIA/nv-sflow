...
def _http_detail(method: str, url: str, status: Optional[int]) -> str:
    """Short last-attempt trace for the summary log: ``GET <url> -> <status>``."""
    return f"{method} {url} -> {status if status is not None else 'no response'}"
...
class HttpGetProbe(Probe):
    ...
    async def check(self, task) -> bool:  # type: ignore[override]
        ...
        self._attempt_detail = _http_detail("GET", self._url, status)
...
class HttpPostProbe(Probe):
    ...
    async def check(self, task) -> bool:  # type: ignore[override]
        ...
        self._attempt_detail = _http_detail("POST", self._url, status)
        return _status_ok(status)
