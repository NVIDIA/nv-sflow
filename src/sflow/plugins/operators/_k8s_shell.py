...
def wait_for_pod_ready_lines(pod_ref: str, ns_seg: str, *, label: str) -> list[str]:
    ...
    Polls the pod phase (as before) but also reads a short ``reason`` (container
    waiting reason, else the ``PodScheduled`` condition message) and echoes
    ``[sflow] <label>: phase=<p> reason=<r> (<n>s)`` to stdout on change or every
    ~30s, so a slow image pull / unschedulable pod is never silent. On the
    timeout fall-through it dumps ``kubectl describe`` + recent events.
    """
    reason_jsonpath = (
    ...
    return [
        ...
        f'    echo "[sflow] {label}: phase=${{phase:-?}} reason=${{reason:-}} ($((i * 2))s)";',
        '    last="$cur";',
        "  fi;",
