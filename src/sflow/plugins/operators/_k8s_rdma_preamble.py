...
``rdma_cm``/verbs node or no ACTIVE port) the preamble sets ``NCCL_IB_DISABLE=1``
so NCCL does not probe dead HCAs. UCX is left alone so it can still pick
cuda_ipc / NVLink on co-located pods (``host_ipc``) instead of being forced onto
Ethernet.
...
_sflow_rdma_tcp() {
  # No usable RDMA: force NCCL onto sockets so it does not probe dead HCAs.
  # Do not set UCX_NET_DEVICES -- UCX must auto-select cuda_ipc/NVLink on
  # co-located pods (host_ipc) or TCP when that is the only path.
  unset NCCL_IB_HCA 2>/dev/null || true
  export NCCL_IB_DISABLE=1
  echo "[sflow-rdma] ${1:-fallback}: using TCP for NCCL (NCCL_IB_DISABLE=1); UCX device selection left to the library"
}
_sflow_rdma_usable() {
...
SFLOW_RDMA_ALL
  ...
  if [ -n "$_hca" ]; then
    ...
    _sflow_rdma_tcp "explicit: no usable RDMA device after topology match"
  fi
}
_sflow_rdma_setup() {
  ...
  if ! _sflow_rdma_usable; then
    _sflow_rdma_tcp "WARNING RDMA requested but unusable in pod (${_SFLOW_RDMA_REASON:-rdmacm/uverbs/ACTIVE port missing})"
    return 0
  fi
  _sflow_gpudirect_check
  case "$_mode" in
...
def build_rdma_affinity_preamble(primary_iface: str = "") -> list[str]:
    ...
    does nothing. When RDMA is not usable in the pod, NCCL is forced to sockets
    (``NCCL_IB_DISABLE=1``) and UCX is left unset. ``primary_iface`` is retained
    for API compatibility but is no longer injected into ``UCX_NET_DEVICES``.
    """
...
class RdmaRuntimeStatus:
    ...
    ``degraded_to_tcp`` is True when at least one pod printed the TCP-fallback
    marker -- RDMA was provisioned by the backend but not usable inside the pod
    (missing ``rdma_cm``/verbs node, or no ACTIVE port). ``reason`` is the
    ...
    how many replicas degraded. ``ucx_intra_node_tcp`` is True only when UCX's debug
...
def parse_rdma_runtime_status(log_text: str) -> RdmaRuntimeStatus | None:
    ...
    injected -- e.g. ``rdma: off`` or a provider that pins a build-time NIC).
    ...
    for raw in log_text.splitlines():
        ...
        if _TCP_SENTINEL in body:
            degraded_reasons.append(body.split(f": {_TCP_SENTINEL}", 1)[0].strip())
            ...
            disabled += 1
    total = len(degraded_reasons) + active + disabled
    ...
    return RdmaRuntimeStatus(
        degraded_to_tcp=bool(degraded_reasons),
        reason=degraded_reasons[0] if degraded_reasons else "",
        pods_degraded=len(degraded_reasons),
        pods_total=total,
