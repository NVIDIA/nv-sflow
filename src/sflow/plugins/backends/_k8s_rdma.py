...
class RdmaPlan:
    ...
    # ``_k8s_rdma_preamble``) instead of a build-time slot. Only valid for
    ...
    def disabled(cls) -> "RdmaPlan":
        """``rdma: off`` -- inject nothing; the recipe/cluster handles networking."""
...
class GkeRdmaProvider(RdmaProvider):
    ...
    def build_plan(self, ctx: RdmaDetectContext) -> RdmaPlan:
        # Each GKE NIC index N maps 1:1 to IB device mlx5_N (gpuNrdma0 layout).
        nic_specs = tuple(
