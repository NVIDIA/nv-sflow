...
sanitization, env secrets, the kubectl wrapper script) lives in ``k8s.shell``.
...
# ``k8s.shell.merged_launcher_lines``; parser: ``k8s.lifecycle`` demux. Kept here
...
# Writable emptyDir mounted at the resolved SFLOW_OUTPUT_DIR on K8s, so the driver-host
# output paths are valid + writable in the pod (the driver's disk isn't reachable, and
# a task script referencing $SFLOW_TASK_OUTPUT_DIR would otherwise error). chmod 0777 by
# the ensure-writable init container so non-root images can write to it.
_SFLOW_SCRATCH_VOLUME_NAME = "sflow-scratch"
...
def _fmt_val(value: Any) -> str:
    """Compact one-line repr of a manifest value, for conflict warning messages."""
    text = " ".join(repr(value).split())
    return text if len(text) <= 60 else text[:57] + "..."


# Leaf list paths that are UNION-merged (order-preserving, de-duped) with the user's
# value instead of replaced -- these are set-like, additive fields where an sflow
# entry is functionally required (Linux capabilities: sflow adds ``IPC_LOCK`` for
# RDMA memory pinning). Everything else follows the replace-with-warning rule.
_UNION_LIST_PATHS = frozenset(
    {
...
def _union_list(base: list[Any], extra: Sequence[Any]) -> list[Any]:
    """Order-preserving union of ``base`` + ``extra`` (base entries first)."""
    merged = list(base)
    for item in extra:
...
def _deep_merge_inplace(
    ...
    *,
    ...
    """Recursively merge ``override`` into ``base`` in place; return ``base``.

    Nested dicts merge key-by-key. A leaf whose path is in :data:`_UNION_LIST_PATHS`
    and is a list in both is UNION-merged (order-preserving, de-duped) so sflow's
    required entries (e.g. ``IPC_LOCK``) are never dropped; that case records no
    conflict since nothing is lost. Every other leaf (scalars, lists) is replaced by
    ``override``'s (deep-copied, so a caller's config is never aliased into the
    rendered manifest) -- user-supplied keys win over the sflow-managed manifest.

    When ``conflicts`` is provided, every REPLACED leaf where the user changes an
    existing (sflow-set) value is appended as ``"<path> (was <old> -> now <new>)"``
    so the caller can warn that an sflow-intended setting was overridden.
    """
    for key, value in override.items():
        keypath = f"{path}.{key}" if path else str(key)
        if isinstance(value, Mapping) and isinstance(base.get(key), dict):
            _deep_merge_inplace(
                base[key], value, path=keypath, conflicts=conflicts
            )
        elif (
            keypath in _UNION_LIST_PATHS
            and isinstance(value, list)
            and isinstance(base.get(key), list)
        ):
            # Additive: sflow's entries are kept, user's are appended -> no conflict.
            base[key] = _union_list(base[key], value)
        else:
            if conflicts is not None and key in base and base[key] != value:
                conflicts.append(
                    f"{keypath} (was {_fmt_val(base[key])} -> now {_fmt_val(value)})"
                )
...
def _deep_merged(
    ...
    *,
    ...
    """Return a deep copy of ``base`` with ``override`` deep-merged into it."""
    return _deep_merge_inplace(
        copy.deepcopy(dict(base)), override, path=path, conflicts=conflicts
    )
...
def _gpu_request(
    *,
    ...
    ``device_plugin`` -> a ``gpu_resource_name`` limit (default ``nvidia.com/gpu``),
                         no pod-level claim.
    """
    ...
    if scheduling == "dra" and resource_claim_name:
        ...
        resources["limits"] = {gpu_resource_name: str(gpu_count)}
    return resources, pod_claims
...
def render_reservation_pod_manifest(
    *,
    ...
    container: dict[str, Any] = {
        "name": "reserve",
        "image": placeholder_image or RESERVATION_POD_IMAGE,
...
def render_task_pod(
    *,
    ...
    container_overrides: Mapping[str, Any] | None = None,
    pod_overrides: Mapping[str, Any] | None = None,
    conflicts: list[str] | None = None,
    sflow_scratch_dir: str | None = None,
) -> dict[str, Any]:
    ...
    # Writable emptyDir mounted at the resolved SFLOW_OUTPUT_DIR (K8s only): the driver's
    # host output dir isn't reachable in the pod, so mounting an emptyDir at that same
    # path makes the driver-host SFLOW_* paths valid + writable WITHOUT remapping the
    # env. The init container chmods it 0777 for non-root images (the entrypoint
    # mkdir -p's the per-task subdir under it).
    if sflow_scratch_dir:
        volumes.append({"name": _SFLOW_SCRATCH_VOLUME_NAME, "emptyDir": {}})
        volume_mounts.append(
            {"name": _SFLOW_SCRATCH_VOLUME_NAME, "mountPath": sflow_scratch_dir}
        )
        scratch_init_root = f"{_ENSURE_WRITABLE_DIR}/{_SFLOW_SCRATCH_VOLUME_NAME}"
        ensure_writable_mounts.append(
            {"name": _SFLOW_SCRATCH_VOLUME_NAME, "mountPath": scratch_init_root}
        )
        ensure_writable_cmds.append(f"chmod 0777 '{scratch_init_root}' || true")
    ...
    # Allocate a pseudo-TTY so tools that redraw a line with ``\r`` (progress bars:
    # aiperf/pip/docker pull) flush live to ``kubectl logs`` -> ``<task>.log``.
    # Without a TTY the container runtime batches newline-less output (surfacing it
    # only in ~16KB chunks or at the final newline), so a long-running progress bar
    # appears all at once. ``stdin: true`` is required for the kubelet to allocate
    # and keep the TTY open. Note: a TTY merges stderr into stdout and keeps raw
    # control bytes (``\r``/ANSI) in the log.
    if tty:
    ...
    managed_sctx: dict[str, Any] = {}
    if run_as_root:
        managed_sctx["runAsUser"] = 0
        managed_sctx["runAsGroup"] = 0
    if rdma_ipc_lock:
        managed_sctx["capabilities"] = {"add": ["IPC_LOCK"]}
    if managed_sctx:
        container["securityContext"] = managed_sctx
    if image_pull_policy:
        container["imagePullPolicy"] = image_pull_policy
    if env_secret_name:
        container["envFrom"] = [{"secretRef": {"name": env_secret_name}}]
    # ``extra_env`` (container ``env``) takes precedence over the env Secret
    # (``envFrom``), so callers must only place here what should win over the
    # recipe's workflow variables (e.g. backend-detected network env). MNNVL
    # transport enables are added to ``extra_env`` upstream only for keys the
    # workflow did not set (see the operator's ``_mnnvl_env_defaults``), so an
    # explicit recipe/`-s` value stays solely in the Secret and still wins.
    if extra_env:
        container["env"] = [
    ...
    if rdma_nic_resources:
        ...
        for res in rdma_nic_resources:
            limits[str(res)] = "1"
    # CPU/memory requests+limits (requests-only by default -- see the operator's
    # resources policy). Requests set the cgroup weight, so GPU pods (weighted by
    # cpu_per_gpu x GPUs) win CPU under contention while any pod bursts into idle
    # CPU; limits stay unset unless the recipe opts in. Coexists with the GPU/RDMA
    # limits above.
    if cpu_request or memory_request:
        req = resources.setdefault("requests", {})
        if cpu_request:
            req["cpu"] = str(cpu_request)
        if memory_request:
            req["memory"] = str(memory_request)
    if cpu_limit or memory_limit:
        lim = resources.setdefault("limits", {})
        if cpu_limit:
            lim["cpu"] = str(cpu_limit)
        if memory_limit:
            lim["memory"] = str(memory_limit)
    if resources:
        container["resources"] = resources

    # --- User container-level fields (curated) ---------------------------------
    # Applied after the sflow-managed container keys; the ``container_overrides``
    # escape hatch is deep-merged LAST so it wins over everything here.
    if working_dir:
        container["workingDir"] = str(working_dir)
    if env_vars:
        # Explicit k8s EnvVar entries (support ``valueFrom``: fieldRef /
        # secretKeyRef / configMapKeyRef). Merged with any backend ``extra_env``;
        # on a name clash the user entry wins (and the clobbered sflow value is
        # recorded so the operator can warn).
        by_name: dict[str, Any] = {e["name"]: e for e in container.get("env", [])}
        for entry in env_vars:
            if "name" not in entry:
                raise ValueError(
                    f"k8s operator 'env' entry is missing the required 'name' key: "
                    f"{entry!r}"
                )
            name = str(entry["name"])
            new_entry = copy.deepcopy(dict(entry))
            if conflicts is not None and name in by_name and by_name[name] != new_entry:
                conflicts.append(
                    f"container.env[{name}] "
                    f"(was {_fmt_val(by_name[name])} -> now {_fmt_val(new_entry)})"
                )
            by_name[name] = new_entry
        container["env"] = list(by_name.values())
    if ports:
        container["ports"] = [copy.deepcopy(dict(p)) for p in ports]
    if lifecycle:
        container["lifecycle"] = copy.deepcopy(dict(lifecycle))
    if security_context:
        # Deep-merge over sflow's managed securityContext (runAsUser/Group +
        # IPC_LOCK); user keys win per-key.
        container["securityContext"] = _deep_merged(
    ...
    if container_overrides:
        _deep_merge_inplace(
            container, container_overrides, path="container", conflicts=conflicts
        )
    ...
    # --- User pod-level fields (curated) ---------------------------------------
    # Applied after the sflow-managed pod keys; the ``pod_overrides`` escape hatch
    # is deep-merged LAST so it wins over everything here.
    if image_pull_secrets:
        pod_spec["imagePullSecrets"] = [{"name": str(s)} for s in image_pull_secrets]
    if service_account:
        pod_spec["serviceAccountName"] = str(service_account)
    if runtime_class:
        pod_spec["runtimeClassName"] = str(runtime_class)
    if priority_class:
        pod_spec["priorityClassName"] = str(priority_class)
    if termination_grace_period is not None:
        pod_spec["terminationGracePeriodSeconds"] = int(termination_grace_period)
    if pod_security_context:
        pod_spec["securityContext"] = _deep_merged(
    ...
    if pod_overrides:
        _deep_merge_inplace(
            pod_spec, pod_overrides, path="pod.spec", conflicts=conflicts
        )

    # sflow's own selector labels (task/allocation) are set first; user ``labels``
    # then merge over them (user wins), and any collision with an sflow selector
    # label is recorded so the operator can warn it was overridden.
    labels: dict[str, str] = {}
    if task_label:
        labels[SFLOW_TASK_LABEL] = task_label
    if allocation_id:
    ...
    if pod_labels:
        for key, value in pod_labels.items():
            ...
            if conflicts is not None and key in labels and labels[key] != value:
                conflicts.append(
                    f"metadata.labels.{key} "
                    f"(was {_fmt_val(labels[key])} -> now {_fmt_val(value)})"
                )
            labels[key] = value
    return {
        "apiVersion": "v1",
