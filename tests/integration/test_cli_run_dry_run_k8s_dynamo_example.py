...
def _run_dry(tmp_path: Path, extra_sets: list[str]):
    ...
    cfg = repo_root / "examples" / "self_contained/kubernetes/dynamo_trtllm_disagg.yaml"
    assert cfg.exists()
...
#
# ``needs_local_model`` passes a ``-a LOCAL_MODEL_PATH=fs://<tmp>`` override so the dry-run
# exercises the artifact-override path. Every K8S serve recipe now exposes the model as a
# ``LOCAL_MODEL_PATH`` fs:// artifact (default in the always-mounted model-store PVC), so
# each dry-runs with OR without the override; we set it True to cover the override path.
@pytest.mark.parametrize(
    "recipe,server_markers,needs_local_model",
    [
        ("self_contained/kubernetes/dynamo_trtllm_agg.yaml", ["agg_server_0"], True),
        ("self_contained/kubernetes/dynamo_sglang_agg.yaml", ["agg_server_0"], True),
        ("self_contained/kubernetes/dynamo_vllm_agg.yaml", ["agg_server_0"], True),
        (
            "self_contained/kubernetes/dynamo_sglang_disagg.yaml",
            ["prefill_server_0", "decode_server_0"],
            True,
        ),
        (
            "self_contained/kubernetes/dynamo_vllm_disagg.yaml",
            ["prefill_server_0", "decode_server_0"],
            True,
        ),
    ],
)
def test_k8s_dynamo_recipe_dry_runs(
    tmp_path: Path, recipe, server_markers, needs_local_model
):
    repo_root = Path(__file__).resolve().parents[2]
    ...
    if needs_local_model:
        model_dir = tmp_path / "model"
        model_dir.mkdir()
        args += ["--artifact", f"LOCAL_MODEL_PATH=fs://{model_dir}"]
    args += [
        "--workspace-dir",
        str(tmp_path),
        "--output-dir",
