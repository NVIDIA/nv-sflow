...
--kube-namespace / --extra-kubectl-args / --kube-compute-domain-*) flow through
``sflow run`` to the kubernetes backend, keeping the recipe cluster-agnostic."""
...
def test_run_dry_run_threads_kube_cli_flags(tmp_path: Path):
    ...
    cfg = repo_root / "examples" / "self_contained/kubernetes/hello_world.yaml"
    assert cfg.exists()
...
def test_run_dry_run_threads_kube_compute_domain_flags(tmp_path: Path):
    # --kube-compute-domain-channel / --kube-compute-domain-create override the recipe's
    # Multi-Node NVLink (ComputeDomain) settings without editing the file.
    repo_root = Path(__file__).resolve().parents[2]
    cfg = repo_root / "examples" / "self_contained/kubernetes/hello_world.yaml"
    assert cfg.exists()
    ...
    assert result.exit_code == 0, result.output
    # Both overrides surface in the kubernetes backend's dry-run details.
    assert "use_compute_domain_channel: auto" in result.output
    assert "create_compute_domain: True" in result.output
