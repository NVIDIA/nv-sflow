...
def test_full_sample_submit_mode_uses_sample_test_for_infmax_tracking():
    ...
    # The exclude list is CI-configurable via $SLURM_E2E_EXCLUDE_NODES
    # (.gitlab-ci.yml pins the live broken-node set), with a fallback default for
    # local/manual runs so those still steer clear of the historically-bad nodes.
    assert (
        'E2E_EXCLUDE_NODES="${SLURM_E2E_EXCLUDE_NODES:-gb-nvl-137-compute02,gb-nvl-137-compute14}"'
        in submit_block
    )
    # Flag and value must be separate argv tokens; a glued "-e --exclude=..."
    # token folds a leading space into the value, which the secondary-backend
    # salloc then can't parse (node exclude silently dropped).
    assert '"-e" "--exclude=$E2E_EXCLUDE_NODES"' in submit_block
    assert "-e --exclude=$E2E_EXCLUDE_NODES" not in submit_block
...
def test_full_sample_and_sample_test_support_min_submit_mode():
    ...
    assert 'sflow run "$EXAMPLES_DIR/self_contained/local/dag.yaml"' in full_sample
    assert 'run_check "run local_variable_domain (live, optional)"' in full_sample
    assert 'local/dag.yaml' not in min_self_contained_block
    assert 'local/variable_domain.yaml' not in min_self_contained_block
    assert 'self_contained/slurm/auto_replica.yaml' in min_self_contained_block
    assert 'self_contained/slurm/dynamo_trtllm_disagg.yaml' in min_self_contained_block
    assert 'self_contained/slurm/resource_release_after.yaml' in min_self_contained_block
    assert 'self_contained/slurm/trtllm_serve_disagg.yaml' in min_self_contained_block
...
def test_disagg_recipes_monitor_servers_from_benchmark():
    ...
    disagg = sorted(Path("examples/self_contained/slurm").glob("*disagg.yaml"))
    assert disagg, "expected *disagg.yaml recipes under examples/self_contained/slurm/"
    for recipe in disagg:
        ...
        sample = Path("src/sflow/samples/self_contained/slurm") / recipe.name
...
def test_self_contained_benchmark_recipes_monitor_servers():
    ...
    checked = 0
    for recipe in sorted(Path("examples/self_contained/slurm").glob("*.yaml")):
        data = yaml.safe_load(recipe.read_text())
...
def test_full_sample_covers_kubernetes_dynamo_and_standalone_examples():
    ...
    # the vLLM/SGLang Dynamo recipes (plain-k8s dynamo loop) and the TRT-LLM recipes
    # (k8s_mpi loop: merged agg, disagg, + bare trtllm-serve agg/disagg). The
    # mimic-script smoke recipes (pd_smoke / apply_launch / multinode_smoke /
    # log_offload_smoke) are NOT examples -- they moved to
    # tests/integration/recipes/kubernetes/ and are covered by pytest
    # (test_cli_run_dry_run_k8s_examples.py), so they must NOT appear in the script.
    script = Path("scripts/full_sample_tests.sh").read_text()

    for recipe, workflow in [
        ("dynamo_vllm_agg", "dynamo_vllm_agg"),
        ("dynamo_vllm_disagg", "dynamo_vllm_disagg"),
        ("dynamo_trtllm_agg", "dynamo_trtllm_agg"),
        ("dynamo_trtllm_disagg", "dynamo_trtllm_disagg"),
        ("dynamo_sglang_agg", "dynamo_sglang_agg"),
        ("dynamo_sglang_disagg", "dynamo_sglang_disagg"),
        ("trtllm_serve_agg", "trtllm_serve_agg"),
        ("trtllm_serve_disagg", "trtllm_serve_disagg"),
    ]:
        assert f"self_contained/kubernetes/{recipe}:{workflow}" in script
    # The templated dynamo run_check title.
    assert "dry-run ${k8s_dynamo_example} uses k8s operator" in script
    # The relocated smoke fixtures are gone from the sample-tests script.
    assert "pd_smoke" not in script
    assert "apply_launch" not in script
    assert "multinode_smoke" not in script
    assert "log_offload_smoke" not in script


def test_full_sample_covers_k8s_recipe_golden_manifests():
    # The preflight must render the representative K8s recipes offline and diff them
    # against checked-in goldens, so preflight_cli covers K8s MANIFEST rendering (not
    # just that recipes plan). Backed by tests/unit/test_k8s_recipe_golden_manifests.py.
    script = Path("scripts/full_sample_tests.sh").read_text()

    assert "k8s recipe golden manifests match offline render" in script
    assert "python -m pytest" in script
    assert "tests/unit/test_k8s_recipe_golden_manifests.py" in script
    # The golden test + at least one golden must exist and be non-trivial.
    assert Path("tests/unit/test_k8s_recipe_golden_manifests.py").is_file()
    goldens = sorted(Path("tests/unit/golden/k8s_recipes").glob("*.yaml"))
    assert len(goldens) >= 3, f"expected >=3 k8s recipe goldens, found {goldens}"
    for golden in goldens:
        assert "kind: Pod" in golden.read_text(), f"{golden} has no rendered Pod"
...
def test_full_sample_covers_cli_backend_extra_arg_and_kube_flags():
    ...
    assert (
        "dry-run --kube-compute-domain-channel/--kube-compute-domain-create override the recipe"
        in script
    )
    assert "use_compute_domain_channel: auto" in script
    assert "create_compute_domain: True" in script
...
def test_full_sample_covers_tui_live_run_with_local_dag():
    ...
    assert 'sflow run "$EXAMPLES_DIR/self_contained/local/dag.yaml"' in tui_block
...
def test_full_sample_covers_result_parsing_sample_yaml():
    ...
    assert 'RESULT_PARSING_SAMPLE="$EXAMPLES_DIR/self_contained/local/result_parsing.yaml"' in result_block
...
def test_full_sample_covers_multi_backend_slurm():
    ...
    assert "self_contained/slurm/multi_backend.yaml" in block
...
def test_sample_test_runs_real_multi_backend():
    ...
    assert 'sflow batch "$EXAMPLES_DIR/self_contained/slurm/multi_backend.yaml"' in script
    ...
    assert "multi_backend.yaml)" in script
