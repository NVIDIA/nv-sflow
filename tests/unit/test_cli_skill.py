...
class TestSkillsCopy:
    ...
    def test_copy_to_custom_dir(self, tmp_path):
        ...
        result = runner.invoke(app, ["skill", "-o", str(output_dir)])
        assert result.exit_code == 0
    ...
    def test_copy_refuses_overwrite_without_force(self, tmp_path):
        output_dir = tmp_path / "skills"
        output_dir.mkdir()
        result = runner.invoke(app, ["skill", "-o", str(output_dir)])
    ...
    def test_copy_overwrites_with_force(self, tmp_path):
        ...
        result = runner.invoke(app, ["skill", "-o", str(output_dir), "--force"])
        assert result.exit_code == 0
        assert "Skills copied to" in result.output
