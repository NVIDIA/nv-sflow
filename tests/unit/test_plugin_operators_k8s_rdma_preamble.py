...
"""Tests for the runtime GPU<->RDMA NIC affinity preamble (``k8s.rdma_preamble``).

String-shape checks cover the generated bash; ``bash -n`` validates syntax; and
functional tests exercise the "no usable RDMA NIC" path deterministically via the
``SFLOW_*_DIR`` test hooks -- no real cluster needed. The key invariant here is
that sflow does NOT force a transport fallback on an unusable NIC (forcing sockets
would also suppress the rack-NVLink/MNNVL path NCCL/UCX auto-detect); it only
hints. So the functional tests assert the env is left UNTOUCHED and the hint is
printed.
"""
...
def test_covers_modes_verify_and_hint():
    ...
    # No usable NIC => HINT only. sflow must NOT force-export the socket-fallback
    # envs (that would also disable rack NVLink/MNNVL that NCCL/UCX auto-detect).
    # Check for an actual `export` statement (line-anchored) -- the envs DO appear
    # in a comment + the hint echo as guidance, which is fine.
    assert "no usable RDMA NIC" in text
    stmts = [ln.strip() for ln in text.splitlines()]
    for _env in ("NCCL_IB_DISABLE", "NCCL_NET_PLUGIN", "NCCL_IBEXT_DISABLE"):
        assert not any(ln.startswith(f"export {_env}") for ln in stmts), _env
    # ...but it DOES name them in the hint so the user can set them if needed.
    assert (
        "NCCL_IB_DISABLE=1 NCCL_NET_PLUGIN=none NCCL_IBEXT_DISABLE=1" in text
    )
...
@_needs_bash
def test_no_usable_nic_when_rdmacm_missing_does_not_force_env(fake_process, tmp_path):
    # Empty device dir -> no rdma_cm -> HINT only; NCCL env left UNTOUCHED so the
    # libraries still auto-detect NVLink/MNNVL (sflow does not force sockets here).
    proc = _run_bash(
        fake_process,
        [_BASH, "-c", _script("enP5p9s0", _TRAILER)],
        env={
    ...
    assert proc.returncode == 0, proc.stderr
    # No transport forced: NCCL_IB_DISABLE / NCCL_NET_PLUGIN stay unset.
    assert "RESULT ucx=[] rails=[] ibhca=[] dis=[] netplugin=[]" in proc.stdout
    assert "no usable RDMA NIC" in proc.stdout


@_needs_bash
def test_no_usable_nic_when_no_active_port_does_not_force_env(fake_process, tmp_path):
    # rdmacm + uverbs present but no IB device has an ACTIVE port -> hint, no force.
    env = _usable_dirs(tmp_path, active=False)
    proc = _run_bash(
        fake_process, [_BASH, "-c", _script("enP5p9s0", _TRAILER)], env=env
    )
    assert proc.returncode == 0, proc.stderr
    assert "dis=[] netplugin=[]" in proc.stdout
    assert "no usable RDMA NIC" in proc.stdout
...
@_needs_bash
def test_unusable_nic_hint_states_all_ports_down_reason(fake_process, tmp_path):
    # The hint marker must name the specific cause so users can triage; the parser
    # must then recover that reason from the emitted line (round-trip).
    env = _usable_dirs(tmp_path, active=False)
    proc = _run_bash(
        fake_process, [_BASH, "-c", _script("enP5p9s0")], env=env
    )
    assert proc.returncode == 0, proc.stderr
    assert "all ports DOWN" in proc.stdout
    status = parse_rdma_runtime_status(proc.stdout)
...
@_needs_bash
def test_unusable_nic_hint_states_missing_rdma_cm_reason(fake_process, tmp_path):
    proc = _run_bash(
        fake_process,
...
# ---------------------------------------------------------------------------
# Runtime status parser (used by sflow to surface an unusable in-pod RDMA NIC)
# ---------------------------------------------------------------------------


def test_parse_detects_unusable_nic_with_reason():
    log = (
        "[pod/decode-server-0/decode-server-0] [sflow-rdma] no InfiniBand port "
        "is ACTIVE (all ports DOWN): no usable RDMA NIC -- leaving transport to "
        "NCCL/UCX auto-detect; to force sockets set NCCL_IB_DISABLE=1 "
        "NCCL_NET_PLUGIN=none NCCL_IBEXT_DISABLE=1 for your cluster\n"
    )
    status = parse_rdma_runtime_status(log)
    assert status is not None
    assert status.rdma_nic_unusable is True
...
def test_parse_reason_keeps_internal_colons():
    log = "[sflow-rdma] a: b: c: no usable RDMA NIC -- leaving transport to lib\n"
    status = parse_rdma_runtime_status(log)
    assert status is not None and status.reason == "a: b: c"


def test_parse_rdma_active_is_not_flagged():
    log = (
    ...
    status = parse_rdma_runtime_status(log)
    assert status is not None
    assert status.rdma_nic_unusable is False
    assert status.pods_degraded == 0 and status.pods_total == 1


def test_parse_explicit_pin_is_not_flagged():
    log = "[sflow-rdma] explicit: NCCL_IB_HCA=mlx5_0 (UCX device selection left to the library)\n"
    status = parse_rdma_runtime_status(log)
    assert status is not None and status.rdma_nic_unusable is False
...
def test_parse_disabled_is_not_flagged():
...
def test_parse_counts_multiple_pods_mixed():
    log = (
        "[pod/a/a] [sflow-rdma] rc: no usable RDMA NIC -- leaving transport to lib\n"
        "[pod/b/b] [sflow-rdma] auto: letting NCCL/UCX auto-select devices\n"
        "[pod/c/c] [sflow-rdma] rc2: no usable RDMA NIC -- leaving transport to lib\n"
    )
    status = parse_rdma_runtime_status(log)
    assert status is not None
    assert status.rdma_nic_unusable is True
