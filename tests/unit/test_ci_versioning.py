...
def _run_feature_branch_version_script(
    *,
    tmp_path: Path,
    branch: str,
    fp,
    ...
    version_block = next(
        item for item in publish_script if ".feature_base_version" in item
    )
    script_lines = [
        line
        for line in version_block.splitlines()
        if "git fetch --tags" not in line and "python -m build" not in line
    ]
    script = "\n".join(
        [
            "set -euo pipefail",
...
def test_feature_branch_version_strips_uppercase_base_tag(tmp_path: Path, fp):
    result = _run_feature_branch_version_script(
        tmp_path=tmp_path,
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    assert "Feature branch base tag: V1.2.3" in result.stdout
    assert result.stdout.endswith("1.2.3.dev42+feature.abc1234")


def test_develop_branch_version_uses_next_patch_dev(tmp_path: Path, fp):
    result = _run_feature_branch_version_script(
        tmp_path=tmp_path,
        branch="develop",
        fp=fp,
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    assert "Develop branch base tag: V1.2.3" in result.stdout
    assert result.stdout.endswith("1.2.4.dev42+feature.abc1234")
