...
def test_srun_operator_splits_bundled_extra_args():
    """srun was the one extra_args consumer that skipped normalize_extra_args.

    A bundled entry then reached srun as a SINGLE argv token it cannot parse, and
    srun ignores what it cannot parse -- so the flags were silently dropped rather
    than rejected. Every other consumer (slurm/docker/k8s backends, docker_run,
    kubectl_config) normalizes; this one is now consistent with them.
    """
    op = SrunOperator(
        SrunOperatorConfig(name="op_srun", extra_args=["--gres=gpu:8 --exclusive"])
    )
    argv = op.build_command(task_name="t1", script=["echo hi"], envs={}).as_list()

    assert "--gres=gpu:8" in argv
    assert "--exclusive" in argv
    assert "--gres=gpu:8 --exclusive" not in argv, "must not stay one token"
    # Still ahead of the payload, which is what srun requires.
    assert argv.index("--exclusive") < argv.index("bash")


def test_srun_operator_extra_args_cannot_delete_an_sflow_managed_flag():
    """add_opt(name) defaults to append=False, which REMOVES every existing option
    of that name (core/command.py). Routed through it, a user flag that collided
    with an sflow-managed one silently deleted sflow's own value instead of
    sitting alongside it. add_arg appends, so both survive and srun decides.
    """
    op = SrunOperator(
        SrunOperatorConfig(
            name="op_srun", partition="batch", extra_args=["--partition", "other"]
        )
    )
    argv = op.build_command(task_name="t1", script=["echo hi"], envs={}).as_list()

    assert "batch" in argv, "sflow's own --partition must survive"
