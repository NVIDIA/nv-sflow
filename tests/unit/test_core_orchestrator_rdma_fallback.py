...
"""The orchestrator surfaces an in-pod RDMA->TCP fallback when a task goes READY.
...
``network_fallback_status``) and, if the pod degraded to slow TCP, emits a WARNING
and records it in the run summary. RDMA-active / unknown tasks stay silent.
"""
...
from sflow.plugins.operators._k8s_rdma_preamble import RdmaRuntimeStatus
...
def test_readiness_surfaces_rdma_tcp_fallback():
    status = RdmaRuntimeStatus(
        degraded_to_tcp=True,
    ...
    assert "TCP" in msg and "all ports DOWN" in msg
    warnings = [
...
def test_readiness_no_warning_when_rdma_active():
    status = RdmaRuntimeStatus(
        degraded_to_tcp=False, reason="", pods_degraded=0, pods_total=1
    )
    orch, server, summary = _mk(status)
    asyncio.run(orch._run_probe(server.probes[0], server))
    assert server.status == TaskStatus.READY
...
def test_readiness_ucx_intra_node_tcp_includes_remedy_hint():
    ...
    asyncio.run(orch._run_probe(server.probes[0], server))
    _name, msg = summary.network[0]
    assert "IMEX" in msg or "use_compute_domain_channel" in msg
