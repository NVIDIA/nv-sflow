...
def test_task_pod_does_not_inject_mnnvl_env():
    # render_task_pod is a dumb renderer: it only emits container env from extra_env.
    # MNNVL transport enables (NCCL_MNNVL_ENABLE / UCX_CUDA_IPC_ENABLE_MNNVL) are
    # decided by the operator as env-Secret-respecting defaults, NOT here -- even
    # when the pod joins a ComputeDomain -- so a recipe/`-s` value in the env Secret
    # is never clobbered by a same-named container env.
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg", scheduling="dra",
        per_pod_gpus=2, resource_claim_name="t-gpu",
        compute_domain_channel="cd-channel",
    )
    env = {e["name"]: e["value"] for e in m["spec"]["containers"][0].get("env", [])}
    assert "NCCL_MNNVL_ENABLE" not in env
    assert "UCX_CUDA_IPC_ENABLE_MNNVL" not in env
    # Claiming the channel IS render's job, and is unaffected.
    claims = m["spec"].get("resourceClaims", [])
    assert any(c.get("resourceClaimTemplateName") == "cd-channel" for c in claims)
...
def test_task_pod_no_cpu_memory_resources_by_default():
    # render_task_pod is a dumb renderer: without cpu/memory args it emits no
    # requests/limits (the operator decides the values and passes them in).
    m = render_task_pod(pod_name="t", image="img:1", configmap_name="t-cfg")
    assert "resources" not in m["spec"]["containers"][0]


def test_task_pod_cpu_memory_requests_and_limits():
    # requests + optional limits land in the right sub-dicts; CPU request coexists
    # with the GPU limit without clobbering it.
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        scheduling="device_plugin", per_pod_gpus=2,
        cpu_request="16", memory_request="32Gi",
        cpu_limit="24", memory_limit="48Gi",
    )
    res = m["spec"]["containers"][0]["resources"]
    assert res["requests"] == {"cpu": "16", "memory": "32Gi"}
    assert res["limits"] == {"nvidia.com/gpu": "2", "cpu": "24", "memory": "48Gi"}


def test_task_pod_cpu_request_only_no_limit():
    # The default policy: a CPU request but no cpu limit (Burstable, no throttling).
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg", cpu_request="4",
    )
    res = m["spec"]["containers"][0]["resources"]
...
def test_task_pod_defaults_no_tty():
    # No TTY by default: the container omits stdin/tty (non-TTY pipe stdout).
    m = render_task_pod(pod_name="t", image="img:1", configmap_name="t-cfg")
    c = m["spec"]["containers"][0]
    assert "tty" not in c and "stdin" not in c


def test_task_pod_tty_allocates_stdin_and_tty():
    # tty=True -> container stdin+tty so \r progress bars stream live to kubectl logs
    # instead of being batched by the container runtime's newline handling.
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg", tty=True
    )
    c = m["spec"]["containers"][0]
...
def test_task_pod_curated_container_fields():
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        working_dir="/work",
        env_vars=[
        ...
        lifecycle={"preStop": {"exec": {"command": ["sh", "-c", "sleep 1"]}}},
        security_context={"readOnlyRootFilesystem": True},
    )
    c = m["spec"]["containers"][0]
    assert c["workingDir"] == "/work"
    names = {e["name"]: e for e in c["env"]}
    assert names["FOO"]["value"] == "bar"
    assert names["POD_IP"]["valueFrom"]["fieldRef"]["fieldPath"] == "status.podIP"
    assert c["ports"] == [{"containerPort": 8000, "name": "http"}]
    assert c["lifecycle"]["preStop"]["exec"]["command"] == ["sh", "-c", "sleep 1"]
    assert c["securityContext"]["readOnlyRootFilesystem"] is True


def test_task_pod_security_context_deep_merges_over_managed():
    # run_as_root sets runAsUser/Group=0; the user securityContext merges over it.
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        run_as_root=True,
...
def test_task_pod_capabilities_add_union_keeps_managed_ipc_lock():
    # rdma_ipc_lock sets managed capabilities.add=[IPC_LOCK]; a user add that omits it
    # must still keep IPC_LOCK (union + de-dup) and record NO conflict (nothing lost).
    conflicts: list[str] = []
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        rdma_ipc_lock=True,
    ...
    caps = m["spec"]["containers"][0]["securityContext"]["capabilities"]
    assert caps["add"] == ["IPC_LOCK", "SYS_PTRACE"]  # managed first, user appended
    assert conflicts == []


def test_task_pod_capabilities_union_dedups_and_via_overrides():
    # Explicitly re-including IPC_LOCK doesn't duplicate it, and the same union applies
    # whether set via security_context or the container_overrides escape hatch.
    for kw in (
        {"security_context": {"capabilities": {"add": ["IPC_LOCK", "SYS_PTRACE"]}}},
        {"container_overrides": {"securityContext": {"capabilities": {"add": ["SYS_PTRACE"]}}}},
    ):
        conflicts: list[str] = []
        m = render_task_pod(
            pod_name="t", image="img:1", configmap_name="t-cfg",
            rdma_ipc_lock=True, conflicts=conflicts, **kw,
        )
        caps = m["spec"]["containers"][0]["securityContext"]["capabilities"]
        assert caps["add"] == ["IPC_LOCK", "SYS_PTRACE"]  # no duplicate IPC_LOCK
        assert conflicts == []


def test_task_pod_non_capability_lists_still_replace_and_warn():
    # Only capabilities.add/drop union; other sflow-set lists keep replace-with-warning.
    conflicts: list[str] = []
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        tolerations=[{"key": "nvidia.com/gpu", "operator": "Exists"}],
        pod_overrides={"tolerations": [{"key": "user", "operator": "Exists"}]},
        conflicts=conflicts,
    )
    assert m["spec"]["tolerations"] == [{"key": "user", "operator": "Exists"}]  # replaced
    assert any("pod.spec.tolerations" in c for c in conflicts)


def test_task_pod_env_vars_user_wins_over_extra_env():
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        extra_env={"FOO": "backend"},
        env_vars=[{"name": "FOO", "value": "user"}],
    )
    env = {e["name"]: e for e in m["spec"]["containers"][0]["env"]}
    assert env["FOO"]["value"] == "user"


def test_task_pod_curated_pod_fields():
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        image_pull_secrets=["regcred", "other"],
        service_account="sa-1",
        runtime_class="nvidia",
        priority_class="high",
        termination_grace_period=45,
        pod_labels={"team": "mlperf"},
        pod_annotations={"note": "hi"},
    ...
    assert m["metadata"]["labels"]["team"] == "mlperf"
    assert m["metadata"]["annotations"] == {"note": "hi"}


def test_task_pod_user_labels_override_sflow_labels_with_conflict():
    # Policy: user value wins on collision; the clobbered sflow selector label is
    # recorded so the operator can warn an sflow-intended value was overridden.
    conflicts: list[str] = []
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        task_label="mytask", allocation_id="alloc-1",
        pod_labels={"sflow.ai/task": "HIJACK", "team": "x"},
        conflicts=conflicts,
    )
    labels = m["metadata"]["labels"]
    assert labels["sflow.ai/task"] == "HIJACK"  # user wins
    assert labels["team"] == "x"                # brand-new label, no conflict
    assert any("metadata.labels.sflow.ai/task" in c for c in conflicts)
    assert not any("team" in c for c in conflicts)


def test_task_pod_conflicts_recorded_for_sflow_overrides():
    conflicts: list[str] = []
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        run_as_root=True,
        ...
        env_vars=[
            {"name": "NCCL_SOCKET_IFNAME", "value": "eth1"},
            {"name": "NEW", "value": "v"},
        ],
        security_context={"runAsUser": 1000},
        pod_overrides={"restartPolicy": "OnFailure", "hostPID": True},
        conflicts=conflicts,
    )
    c = m["spec"]["containers"][0]
    env = {e["name"]: e for e in c["env"]}
    assert env["NCCL_SOCKET_IFNAME"]["value"] == "eth1"  # user wins
    assert c["securityContext"]["runAsUser"] == 1000  # user wins over root (0)
    assert m["spec"]["restartPolicy"] == "OnFailure"  # override wins
    assert m["spec"]["hostPID"] is True  # brand-new key
    joined = " ; ".join(conflicts)
    assert "container.env[NCCL_SOCKET_IFNAME]" in joined
    assert "container.securityContext.runAsUser" in joined
    assert "pod.spec.restartPolicy" in joined
    # Additions (new keys) must NOT be reported as conflicts.
    assert "NEW" not in joined and "hostPID" not in joined


def test_task_pod_no_conflicts_when_user_only_adds_new_keys():
    conflicts: list[str] = []
    render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        env_vars=[{"name": "BRANDNEW", "value": "1"}],
        working_dir="/w", service_account="sa",
        conflicts=conflicts,
    )
    assert conflicts == []


def test_task_pod_manifest_unchanged_when_new_fields_unset():
    # Backward-compat: passing the new fields as unset renders byte-identically to
    # not passing them at all (no behavioral change to existing manifests).
    base = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        run_as_root=True, shm_size="8Gi", task_label="x", allocation_id="a",
    )
    same = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        run_as_root=True, shm_size="8Gi", task_label="x", allocation_id="a",
        env_vars=None, working_dir=None, security_context=None, ports=None,
        lifecycle=None, image_pull_secrets=(), service_account=None,
        runtime_class=None, priority_class=None, termination_grace_period=None,
        pod_labels=None, pod_annotations=None, pod_security_context=None,
        container_overrides=None, pod_overrides=None, conflicts=[],
    )
    assert base == same


def test_task_pod_overrides_win_and_deep_merge():
    m = render_task_pod(
        pod_name="t", image="img:1", configmap_name="t-cfg",
        run_as_root=True,
        container_overrides={
            "securityContext": {"privileged": True}, "workingDir": "/ovr"
        },
        pod_overrides={
    ...
    c = m["spec"]["containers"][0]
    # deep-merge: managed runAsUser preserved, override adds privileged
    assert c["securityContext"]["runAsUser"] == 0
    assert c["securityContext"]["privileged"] is True
    assert c["workingDir"] == "/ovr"
    spec = m["spec"]
    assert spec["hostAliases"] == [{"ip": "1.2.3.4", "hostnames": ["h"]}]
    assert spec["restartPolicy"] == "OnFailure"  # override wins over managed default
