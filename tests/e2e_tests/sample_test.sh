...
run_multi_backend_real() {
    ...
    mb_output=$(sflow batch "$EXAMPLES_DIR/slurm_multi_backend.yaml" \
...
if [ "$TEST_TYPE" = "s" ] || [ "$TEST_TYPE" = "a" ] || [ "$TEST_TYPE" = "smoke" ] || [ "$TEST_TYPE" = "min" ]; then
    ...
    if [ "$TEST_TYPE" = "min" ]; then
        MIN_SELF_CONTAINED=(
            "$EXAMPLES_DIR/slurm_auto_replica.yaml"
            "$EXAMPLES_DIR/slurm_dynamo_trtllm_disagg.yaml"
            "$EXAMPLES_DIR/slurm_resource_release_after.yaml"
            "$EXAMPLES_DIR/slurm_trtllm_serve_disagg.yaml"
        )
        MIN_BULK_ARGS=()
